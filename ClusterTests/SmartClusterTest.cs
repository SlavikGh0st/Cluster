...
{
    [TestFixture]
    public abstract class SmartClusterTest
    {
        ...
        public void Client_should_return_success_when_there_is_only_one_fast_replica()
        {
            ...
            LastTurnResultProcessRequests(Timeout);
        }
        ...
        public virtual void Client_should_return_success_when_timeout_is_close()
        {
            ...
            LastTurnResultProcessRequests(Timeout + Epsilon);
        }
        ...
        public void Client_should_timeout_when_all_replicas_are_slow()
        {
            ...
            Action action = () => LastTurnResultProcessRequests(Timeout);
        ...
        protected abstract SmartClusterClientBase CreateClient(string[] replicaAddresses);
        ...
        protected TimeSpan[] LastTurnResultProcessRequests(double timeout, uint turn = 1, int take = 20)
        {
            ...
            var result = new TimeSpan[take];

            Console.WriteLine("Testing {0} started", client.GetType());
            for (var i = 0; i < turn; i++)
            {
                result = Task.WhenAll(Enumerable.Range(0, take).Select(i => i.ToString("x8")).Select(
                    async query =>
                    {
                        ...
                        {
                            var clientResult =
                                await client.ProcessRequestAsync(query, TimeSpan.FromMilliseconds(timeout));
                            timer.Stop();
